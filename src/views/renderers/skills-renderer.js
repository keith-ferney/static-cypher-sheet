...
class SkillsRenderer {
    ...
    static async renderSkills(skills) {
        ...
        const template = await templateLoader.loadTemplate('skill-row');
        container.innerHTML = normalizedSkills.map((skill, idx) => {
            return templateLoader.render(template, {
                name: skill.name || '',
                index: idx,
                powerShift: skill.powerShift || 0,
                poolEmpty: !skill.pool,
                poolMight: skill.pool === 'might',
                poolSpeed: skill.pool === 'speed',
                poolIntellect: skill.pool === 'intellect',
                typeEmpty: !skill.type,
                typeTrained: skill.type === 'trained',
                typeSpecialized: skill.type === 'specialized',
                typeInability: skill.type === 'inability'
            });
        }).join('');
    }
}
